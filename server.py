...
def atomic_write(path, content):
    """Durably replace a data file without exposing a partial write."""
...
async def broadcast(pad, message, exclude=None):
    ...
    if targets:
        await asyncio.gather(
            *[asyncio.wait_for(c.send(message), timeout=5) for c in targets],
            return_exceptions=True,
        )


async def broadcast_roles(pad):
    await asyncio.gather(
        *[
            client.send(
                json.dumps({"type": "role", "editor": client is pad["editor"]})
            )
            for client in pad["clients"]
        ],
        return_exceptions=True,
    )
...
async def handler(websocket):
    ...
    try:
        ...
        if pad["editor"] is None:
            pad["editor"] = websocket
        ...
        async for message in websocket:
            ...
            if data.get("type") == "update":
                ...
                if websocket is not pad["editor"]:
                    await websocket.send(
                        json.dumps(
                            {
                                "type": "readonly",
        ...
        if pad is not None:
            ...
            if pad["editor"] is websocket:
                pad["editor"] = next(iter(pad["clients"]), None)
            if pad["clients"]:
                await broadcast_roles(pad)
